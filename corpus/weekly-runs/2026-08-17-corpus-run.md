...
| **Authorities appearing at all** | **27** | **3** |

**The report no longer has to lead Theme 4 with an absence.** Not *"no EU marketing enforcement has appeared"* — instead: **the EU's consolidated non-compliance record is 98.8% Italian, Italy has licensed 2.8% of the EU's CASPs, the country that licenses the most (nearly a quarter of all of them) appears zero times, and 27 authorities grant authorisations while 3 have ever reported a non-compliant entity.** That is not a thin early sample of a maturing regime. It is one authority's reporting practice.
...
- **🆕 (qq) 🔴 THE RUN'S OWN HEADLINE CARRIED A WRONG FIGURE FOR TWENTY MINUTES, AND THE DEFECT WAS THE ONE THIS RUN DOCUMENTED.** The first draft of this record, the capture file, the findings entry and the `situation.md` line all stated that **4** competent authorities have ever reported a non-compliant entity. **The correct figure is 3 — CONSOB, the AFM and the NBS.** The error came from grouping `NCASP.csv` by raw `ae_competentAuthority` **without normalising whitespace**, and CONSOB appears in that file under **two spellings differing by one space** (164 rows + 1 row). So the count returned 4 authorities where there are 3. **This is precisely source-defect 8, which this run logged in its own capture file — "whitespace inside grouping keys silently splits naive `GROUP BY` counts" — and then committed anyway.** Caught by the verification pass recomputing the figure a third time with `' '.join(x.split())` normalisation, and fixed across all four files before the corrected commit. **Two lessons, and the second is the useful one: (1) documenting a defect is not the same as defending against it — the note and the code must be written in the same pass; (2) the 165/167 = 98.8% CONSOB concentration was RIGHT throughout, because it was computed with a substring match rather than an exact group key. The same file, read two ways, gave one right answer and one wrong one.** Recorded rather than quietly amended: **a run that spent its headline on our own instruments producing false records should not hide that it produced one.**
...
- `corpus/regulator-filings/esma-casps-authorised-register-at-source-2026-08-17.md` — **NEW. The run's headline.** Interim MiCA register file 1/5 at source: 329 rows, **325 distinct CASPs, 27 authorities**; **the CONSOB/BaFin inversion (2.8% authorised / 98.8% of non-compliance vs 21.6% / 0.0%)**; **27 authorities authorise, 3 report non-compliance**; the full **10-firm / 13-entity** cohort cross-match with per-hit adjudication; **0 of 8 foundations and 0 of 5 wallets authorised**; Binance and HTX absent; Aave/Push recorded AMBIGUOUS and not admitted; three `crypto.com` false positives; service-letter distribution (**21 of 325 hold `b.`**); 289-vs-34 pre/post-deadline split; **only 2 withdrawals ever, neither marketing-related**; eleven source defects logged uncorrected; five explicit non-claims.
